...
while [ "${CONNUSED}" != "0" ]
do
  CONNUSED=`mysql -uradmin -pradmin -h127.0.0.1 -P16032 -e 'SELECT IFNULL(SUM(ConnUsed),0) FROM stats_mysql_connection_pool WHERE status="OFFLINE_SOFT" AND srv_host="mysql1"' -B -N 2>/dev/null`
  sleep 0.05
done

echo "Reconfiguring Orchestrator topology and performing failover from mysql1 to mysql2..."
docker exec -ti orc1 bash -c "orchestrator-client -c move-below -i mysql3 -d mysql2"
docker exec -ti orc1 bash -c "orchestrator-client -c graceful-master-takeover -a mysql1"
